"""
src/compare_models_after.py
[수정 후 버전 - 기본 파이프라인] retrieve_diverse() 기준으로 5개 임베딩 모델을 비교. top-k=5.

실행 방법:
    conda activate DL_PY311
    cd src
    python compare_models_after.py
"""
import sys
import time
import os
...
def evaluate(collection, embedder, k):
    ...
    for q in ground_truth_quizzes:
        retrieved = retrieve_diverse(collection, embedder, q["question"], k=k)
...
def main():
    print("=" * 100)
    print("[수정 후 - 기본 파이프라인] 문서 추출 + 청킹 (chunk_size=150) / 정답셋 {}문항 / k={}".format(
    ...
    for cand in CANDIDATES:
        ...
        collection = build_index(
            chunks, embedder,
            persist_dir=os.path.join(THIS_DIR, "chroma_after_" + model_name.replace("/", "_")),
            collection_name="ajin_eval_after",
        )
    ...
    print_table("[[ 수정 후 (retrieve_diverse) - 5개 모델 비교, k={} ]]".format(K), results, K)

    best = max(results, key=lambda m: results[m]["mrr"])
    print()
    print("최종 추천 모델 (MRR 기준): " + best)
